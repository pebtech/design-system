            ...
            {({ close }) => (
                <>
                    <PopoverButton className="outline-none">
                        <div className={`flex w-fit items-center gap-2 rounded-lg px-3 py-1.5 transition-colors cursor-pointer ${value?.startDate ? 'bg-zinc-100 dark:bg-zinc-800' : 'bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-900 dark:hover:bg-zinc-800'}`}>
                            {value?.startDate ? (
                        ...
                        <PopoverPanel className="absolute left-0 z-10 mt-2 w-80 rounded-xl bg-white/95 backdrop-blur-xl shadow-lg ring-1 ring-zinc-950/10 dark:bg-zinc-800/95 dark:ring-white/10 p-4">
                            ...
                            <div className="grid grid-cols-7 gap-1">
                                {(() => {
                                    ...
                                    return [...paddingDays, ...days].map((day, index) => {
                                        ...
                                        const isSelected = value?.startDate && isSameDay(day, value.startDate)
                                        const isEndDate = value?.endDate && isSameDay(day, value.endDate)
                                        const isInRange = value?.startDate && value?.endDate &&
                            ...
                            <div className="mt-4 pt-3 border-t border-zinc-200 dark:border-zinc-700">
                                <p className="text-xs text-zinc-500 text-center">
                                    {selectingEndDate && value?.startDate
                                        ? 'Select end date'
                                        : 'Select start date'}
                                </p>
                            </div>
