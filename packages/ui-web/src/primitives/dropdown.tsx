  ...
  return (
    <Headless.MenuItems
      ...
      className={clsx(
        className,
        // Anchor positioning
        '[--anchor-gap:--spacing(2)] [--anchor-padding:--spacing(1)] data-[anchor~=end]:[--anchor-offset:6px] data-[anchor~=start]:[--anchor-offset:-6px] sm:data-[anchor~=end]:[--anchor-offset:4px] sm:data-[anchor~=start]:[--anchor-offset:-6px]',
        // Base styles
    ...
    >
      {({ ...bag }) => (
        <div className="col-span-full max-h-[var(--anchor-max-height)] overflow-y-auto p-1">
...
export function DropdownItem({
  ...
  const classes = clsx(
    ...
    // Focus
    'data-focus:bg-hover-primary data-focus:text-primary',
    // Disabled state
    ...
    '*:data-[slot=icon]:text-muted data-focus:*:data-[slot=icon]:text-primary dark:*:data-[slot=icon]:text-muted dark:data-focus:*:data-[slot=icon]:text-white',
    // Avatar
...
export function DropdownDescription({
  ...
  return (
    <Headless.Description
      ...
      className={clsx(
        className,
        'col-span-2 col-start-2 row-start-2 text-sm/5 text-muted group-data-focus:text-primary sm:text-xs/5'
      )}
    />
  )
}
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split('')).map((char, index) => (
        <kbd
          key={index}
          className={clsx([
            'min-w-[2ch] text-center font-sans text-muted capitalize group-data-focus:text-primary forced-colors:group-data-focus:text-[HighlightText]',
